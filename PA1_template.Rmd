...
We start by downloading the .zip file from: https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip
...
Next we calculate the total number of steps taken per day using the 'plyr' package and the 
'ddply' function.

We then make a histogram of the total number of steps taken each day and find the mean and median of the total number of steps taken per day.
...
We now plot the Average Number of Steps Taken by 5-minute Intervals across all days.

We also determine and report which interval has the highest step average.
...
```
## Missing Values
We will now replace all NA's with the mean value for that time interval across all 61 days, and then plot the Average Number of Steps Taken by 5-minute Intervals across all days.
...
We now distinguish whether a day is a weekday or weekend day using the weekdays() function.
This gives us the day of the week, which we use to add a factor variable equal to 'weekday' or 'weekend'.
...
We now use the 'lattice' package to make a panel plot of the Average Steps by Time Interval over all days, with separate plots for Weekend Days and WeekDays.
